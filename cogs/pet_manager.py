...
class PetManagerCog(commands.Cog):
    ...
    async def pets(self, ctx):
        ...
        if not pets:
            await ctx.send("You don't have any pets yet! Try getting some from OSRS activities or Pokemon encounters!")
            return
    ...
    async def pet_interact(self, ctx, pet_name: str):
        ...
        if interaction_result["leveled_up"]:
            embed.add_field(name="Level Up!", value=f"{pet.name} grew to level {interaction_result['current_level']}! 🎉", inline=False)
    ...
    async def pet_leaderboard(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="Top Pet Collectors",
            color=discord.Color.gold()
        )
    ...
    async def pet_search(self, ctx, rarity: Optional[str] = None, origin: Optional[str] = None):
        ...
        if rarity:
            try:
                ...
                await ctx.send(f"Invalid rarity! Choose from: {', '.join(r.name for r in PetRarity)}")
                return
        ...
        if origin:
            try:
                ...
                await ctx.send(f"Invalid origin! Choose from: {', '.join(o.name for o in PetOrigin)}")
                return
        ...
        for pet in pets:
            pet_info = (f"Origin: {pet.origin.value.title()}\n"
