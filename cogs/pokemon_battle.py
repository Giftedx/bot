...
class PokemonBattle(commands.Cog):
    ...
    def apply_status_effects(self, pokemon_data: Dict) -> Tuple[int, str]:
        ...
        if status['condition'] == 'burn':
            ...
            if status.get('toxic_counter', 0) > 0:  # Toxic
                damage = int(max_hp * self.status_effects['badly_poison']['damage'] * status['toxic_counter'])
    ...
    async def use_move(self, ctx, move_name: str):
        ...
        try:
            ...
            if 'status' in attacker:
                ...
                if status['condition'] in ['sleep', 'freeze']:
                    ...
                    if random.random() < self.status_effects['paralyze']['skip_chance']:
                        await ctx.send(f"{attacker['pokemon']['name'].title()} is paralyzed and can't move!")
                        # Switch turns
                        battle['current_turn'] = next(uid for uid in battle['trainers'] if uid != ctx.author.id)
                        trainer = ctx.guild.get_member(battle['current_turn'])
                        await ctx.send(f"{trainer.mention}'s turn!")
            ...
            if move_name not in attacker['moves']:
                return await ctx.send(f"Your Pokemon doesn't know that move! Available moves: {', '.join(attacker['moves'])}")
            ...
            if power:
                ...
                damage = int(((2 * level / 5 + 2) * power * attack / defense / 50 + 2) * 
                           stab * type_effectiveness * random.uniform(0.85, 1.0))
            else:
                damage = 0
            ...
            if 'status' in move_data and random.random() < move_data['status_chance']:
                defender['status'] = {
                    ...
                    'turns_left': random.randint(1, self.status_effects.get(move_data['status'], {}).get('max_turns', 1))
                }
                message_parts.append(f"{defender['pokemon']['name'].title()} was {move_data['status']}ed!")
    ...
    async def handle_victory(self, ctx, winner_id):
        ...
        loser_pokemon = battle['trainers'][next(uid for uid in battle['trainers'] if uid != winner_id)]['pokemon']
        ...
        await self.bot.db.execute(
            ...
            new_xp, new_level, winner_pokemon['id']
        )
            ...
            async with session.get(f'https://pokeapi.co/api/v2/pokemon-species/{pokemon_id}') as resp:
                if resp.status == 200:
                    ...
                    for evolution in data.get('evolution_chain', {}).get('evolves_to', []):
                        if evolution.get('evolution_details', [{}])[0].get('min_level', 101) <= level:
                            return evolution
                return None
            ...
            async with session.get(evolution['species']['url']) as resp:
                if resp.status == 200:
                    ...
                    await self.bot.db.execute(
                        ...
                        evolution_data['name'], evolution_data['id'], pokemon_id
                    )
