...
async def test_add_and_get(queue_manager):
    item1 = QueueItem(
        media_id="1", requester_id="user1", timestamp=datetime.now(), priority=QueuePriority.HIGH
    )
    item2 = QueueItem(
        media_id="2", requester_id="user2", timestamp=datetime.now(), priority=QueuePriority.MEDIUM
    )
    item3 = QueueItem(
        media_id="3", requester_id="user3", timestamp=datetime.now(), priority=QueuePriority.LOW
    )
...
async def test_priority_ordering(queue_manager):
    item1 = QueueItem(
        media_id="1", requester_id="user1", timestamp=datetime.now(), priority=QueuePriority.LOW
    )
    item2 = QueueItem(
        media_id="2", requester_id="user2", timestamp=datetime.now(), priority=QueuePriority.HIGH
    )
    item3 = QueueItem(
        media_id="3", requester_id="user3", timestamp=datetime.now(), priority=QueuePriority.MEDIUM
    )
