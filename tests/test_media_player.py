...
async def test_stream_monitor_retry(media_player, mock_voice_channel):
    ...
    process = AsyncMock()
    process.wait.side_effect = [asyncio.TimeoutError(), None]  # Success on second try
    process.returncode = 0
...
async def test_stream_quality_config(media_player, mock_voice_channel):
    ...
    await media_player.play("test.mp4", mock_voice_channel, config=StreamConfig(quality="high"))
