...
def world() -> World:
    """Create a test world."""
    return World(id=301, name="Test World 301", type="normal", region="us")
...
def player() -> Player:
    ...
    return Player(id=1, name="TestPlayer", skills=skills)
...
def high_level_player() -> Player:
    ...
    skills: Dict[SkillType, Skill] = {
        skill_type: Skill(type=skill_type, level=70) for skill_type in SkillType
    }
    return Player(id=2, name="HighLevelPlayer", skills=skills)
...
def test_world_manager_initialization(world_manager: WorldManager) -> None:
    ...
    skill_worlds = world_manager.get_available_worlds(type_filter="skill_total")
    assert len(skill_worlds) > 0


def test_world_joining(world_manager: WorldManager, player: Player) -> None:
...
def test_skill_total_requirements(
    world_manager: WorldManager, player: Player, high_level_player: Player
) -> None:
    """Test skill total world requirements."""
    # Get 1250+ world
    skill_world = next(w for w in world_manager.worlds.values() if "1250+" in w.name)
...
def test_world_action_validation(world_manager: WorldManager, player: Player) -> None:
    """Test world action validation."""
    # Join PvP world
    pvp_world = next(w for w in world_manager.worlds.values() if w.type == "pvp")
    ...
    # Join normal world
    normal_world = next(w for w in world_manager.worlds.values() if w.type == "normal")
...
def test_deadman_restrictions(world_manager: WorldManager, player: Player) -> None:
    """Test deadman mode restrictions."""
    # Join deadman world
    deadman_world = next(w for w in world_manager.worlds.values() if w.type == "deadman")
    ...
    assert not world_manager.validate_world_action(player, "trade", normal_world)


def test_high_risk_restrictions(world_manager: WorldManager, player: Player) -> None:
    """Test high risk world restrictions."""
    # Join high risk world
    high_risk_world = next(w for w in world_manager.worlds.values() if w.type == "high_risk")
