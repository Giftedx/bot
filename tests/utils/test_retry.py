...
def test_retry_backoff():
    ...
    @retry(max_attempts=3, exceptions=ValueError, backoff_base=2.0, backoff_max=4.0, jitter=False)
    def backoff_test():
        nonlocal attempts
        attempts += 1
        raise ValueError("Test error")
...
async def test_circuit_breaker_with_metric():
    ...
    @circuit_breaker(failure_threshold=2, reset_timeout=1.0, metric_name="test_circuit")
    async def failing_service():
        nonlocal failures
        failures += 1
        raise ValueError("Service error")
