...
def test_verify_ffmpeg_failure(mock_find_ffmpeg: MagicMock, ffmpeg_manager: FFmpegManager) -> None:
    ...
    with patch('subprocess.run', side_effect=subprocess.SubprocessError("FFmpeg verification failed")):
        with pytest.raises(FFmpegError) as exc_info:
            ffmpeg_manager._verify_ffmpeg()
...
@pytest.mark.asyncio
@patch("src.core.ffmpeg_manager.FFmpegManager._find_ffmpeg", return_value="fake_path")
async def test_transcode_media_success(mock_find_ffmpeg: MagicMock, ffmpeg_manager: FFmpegManager) -> None:
...
@pytest.mark.asyncio
@patch("src.core.ffmpeg_manager.FFmpegManager._find_ffmpeg", return_value="fake_path")
async def test_transcode_media_failure(mock_find_ffmpeg: MagicMock, ffmpeg_manager: FFmpegManager) -> None:
