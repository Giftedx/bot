...
class RepositoryMetrics:
    ...
    search_latency: Histogram = field(
        default_factory=lambda: Histogram(
            "repo_search_latency_seconds", "Repository search operation latency", ["category"]
        )
    )
    parse_errors: Counter = field(
        default_factory=lambda: Counter(
            "repo_parse_errors_total", "Number of repository parsing errors", ["source"]
        )
    )

    # GitHub API metrics
    github_api_errors: Counter = field(
        default_factory=lambda: Counter(
            "github_api_errors_total", "Number of GitHub API errors", ["type"]
        )
    )
    github_api_latency: Summary = field(
        default_factory=lambda: Summary(
            "github_api_latency_seconds", "GitHub API request latency", ["operation"]
        )
    )
    api_rate_limit: Gauge = field(
        default_factory=lambda: Gauge(
            "github_api_rate_limit", "GitHub API rate limit status", ["type"]
        )
    )

    # Repository metrics
    repositories_total: Gauge = field(
        default_factory=lambda: Gauge(
            "repositories_total", "Total number of repositories", ["category"]
        )
    )
    repository_stars: Summary = field(
        default_factory=lambda: Summary("repository_stars", "Repository star counts", ["category"])
    )
    metadata_updates: Counter = field(
        default_factory=lambda: Counter(
            "metadata_updates_total", "Number of repository metadata updates"
        )
    )

    # Cache metrics
    cache_hits: Counter = field(
        default_factory=lambda: Counter("cache_hits_total", "Number of cache hits", ["type"])
    )
    cache_misses: Counter = field(
        default_factory=lambda: Counter("cache_misses_total", "Number of cache misses", ["type"])
    )
    cache_size: Gauge = field(
        default_factory=lambda: Gauge("cache_size_bytes", "Current cache size in bytes")
    )

    # Task metrics
    task_latency: Summary = field(
        default_factory=lambda: Summary("task_latency_seconds", "Task execution latency", ["name"])
    )
    task_errors: Counter = field(
        default_factory=lambda: Counter(
            "task_errors_total", "Number of task execution errors", ["name"]
        )
    )
    active_tasks: Gauge = field(
        default_factory=lambda: Gauge("active_tasks", "Number of currently active tasks", ["type"])
    )
    queue_size: Gauge = field(
        default_factory=lambda: Gauge("queue_size", "Current queue size", ["type"])
    )

    # Health metrics
    service_health: Gauge = field(
        default_factory=lambda: Gauge(
            "service_health", "Service health status (1=healthy, 0=unhealthy)", ["service"]
        )
    )
    health_check_latency: Summary = field(
        default_factory=lambda: Summary(
            "health_check_latency_seconds", "Health check execution latency", ["check"]
        )
    )
...
class MetricsManager:
    ...
    def __init__(self, port: int = None):
        ...
        # Profiling metrics
        self.profile_duration = Summary(
            "profile_duration_seconds", "Duration of profiled operations", ["name"]
        )
        self.profile_calls = Counter(
            "profile_calls_total", "Number of calls to profiled operations", ["name"]
        )
        self.profile_time_per_call = Summary(
            "profile_time_per_call_seconds",
    ...
    def record_profile_stats(
        self, name: str, duration: float, calls: int, time_per_call: float
    ) -> None:
