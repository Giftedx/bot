...
class Validator:
    ...
    def validate(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if errors:
            raise ValidationError(message="Validation failed", field="multiple", value=errors)
    ...
    def _validate_rule(self, rule: ValidationRule, data: Dict[str, Any]) -> None:
        ...
        if rule.rule_type == "required" and value is None:
            raise ValidationError(
                message=rule.message or f"Field {rule.field} is required", field=rule.field
            )
        ...
        if rule.rule_type == "type":
            if not isinstance(value, rule.value):
                raise ValidationError(
                    ...
                    or f"Field {rule.field} must be of type {rule.value.__name__}",
                    field=rule.field,
                    value=value,
                )
            ...
            if not isinstance(value, (int, float)) or not (rule.value[0] <= value <= rule.value[1]):
                raise ValidationError(
                    message=rule.message
                    or f"Field {rule.field} must be between {rule.value[0]} and {rule.value[1]}",
                    field=rule.field,
                    value=value,
                )
            ...
            if not hasattr(value, "__len__") or not (rule.value[0] <= len(value) <= rule.value[1]):
                raise ValidationError(
                    message=rule.message
                    or f"Field {rule.field} length must be between {rule.value[0]} and {rule.value[1]}",
                    field=rule.field,
                    value=value,
                )
...
def validate_dataclass(instance: Any) -> None:
    ...
    for field in fields(instance):
        ...
        if not field.default and not field.default_factory and value is None:
            errors.append(
                ValidationError(message=f"Field {field.name} is required", field=field.name)
            )
            continue
    ...
    if errors:
        raise ValidationError(message="Dataclass validation failed", field="multiple", value=errors)
...
def validate_type(value: Any, expected_type: Type) -> None:
    ...
    if not isinstance(value, expected_type):
        raise ValidationError(
            message=f"Value must be of type {expected_type.__name__}", value=value
        )
...
def validate_datetime(value: str) -> datetime:
    ...
    try:
        ...
        raise ValidationError(message="Invalid datetime format", value=value, original_error=e)
