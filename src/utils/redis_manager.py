...
class RedisManager:
    ...
    async def cache_set(self, key: str, value: Any, expire: int = 3600) -> None:
        ...
        await self.execute("setex", f"{self.keys.CACHE_PREFIX}{key}", expire, data)

    # Queue operations
    async def queue_push(self, queue_name: str, item: Any) -> None:
        """Push item to queue."""
        await self.execute("rpush", f"{self.keys.QUEUE_PREFIX}{queue_name}", json.dumps(item))

    async def queue_pop(self, queue_name: str, timeout: int = 0) -> Optional[T]:
        """Pop item from queue with optional timeout."""
        result = await self.execute("blpop", f"{self.keys.QUEUE_PREFIX}{queue_name}", timeout)
        if result:
    ...
    async def stream_add(self, stream: str, data: Dict[str, Any]) -> str:
        """Add entry to stream."""
        return await self.execute("xadd", f"{self.keys.STREAM_PREFIX}{stream}", "*", data)

    async def stream_read(
        self, stream: str, count: int = 100, block: int = 0
    ) -> List[Dict[str, Any]]:
        """Read from stream."""
        return await self.execute(
            "xread", count=count, block=block, streams=[f"{self.keys.STREAM_PREFIX}{stream}", "$"]
        )
