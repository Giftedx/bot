...
class BaseCollector:
    ...
    async def fetch_data(
        ...
        while retries < self.config["max_retries"]:
            try:
                ...
                logger.warning(
                    f"Timeout fetching {url}, attempt {retries + 1}/{self.config['max_retries']}"
                )
        ...
        raise Exception(
            f"Failed to fetch data from {url} after {self.config['max_retries']} attempts"
        )
    ...
    async def _save_to_cache(self, key: str, data: Any):
        ...
        if self.redis:
            try:
                self.redis.setex(key, self.config["cache_duration"], json.dumps(data))
                return
            except Exception as e:
    ...
    async def process_batch(self, items: List[Any], batch_size: Optional[int] = None) -> List[Any]:
        ...
        for i in range(0, len(items), batch_size):
            ...
            batch_results = await asyncio.gather(
                *[self.process_item(item) for item in batch], return_exceptions=True
            )
