...
class PokemonDataCollector(BaseCollector):
    ...
    async def collect(self) -> Dict[str, Any]:
        ...
        data = {"api_data": await self._collect_api_data(), "rom_data": {}}
        ...
        has_roms = any(os.path.exists(path) and any(os.listdir(path)) for path in rom_paths if path)
    ...
    async def _collect_api_data(self) -> Dict[str, Any]:
        ...
        api_data = {"pokemon": [], "moves": [], "items": [], "locations": []}
        ...
        async def fetch_batch(urls: List[str], batch_size: int = 5) -> List[Dict]:
            ...
            for i in range(0, len(urls), batch_size):
                ...
                batch_results = await asyncio.gather(
                    *[self.fetch_data(url) for url in batch], return_exceptions=True
                )
        ...
        pokemon_urls = [
            f"{self.config['pokeapi_url']}/pokemon/{i}" for i in range(1, 152)  # Gen 1 Pokemon
        ]
        move_urls = [f"{self.config['pokeapi_url']}/move/{i}" for i in range(1, 166)]  # Gen 1 moves
    ...
    async def _collect_rom_data(self) -> Dict[str, Dict]:
        ...
        for game in self.game_config["games"]:
            rom_path = os.path.join(
                self.config["rom_paths"][game["platform"]], f"{game['name']}.{game['platform']}"
            )
    ...
    async def _parse_gb_pokemon(self, rom_data: bytes) -> List[Dict]:
        ...
        pokemon_list = []
        base_stats_addr = self._find_pattern(
            rom_data,
    ...
    async def _parse_gb_maps(self, rom_data: bytes) -> List[Dict]:
        ...
        maps_list = []
        maps_addr = self._find_pattern(
            rom_data, b"\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00"
        )
        ...
        if maps_addr:
            ...
            while current_addr < len(rom_data) - self.rom_specs["gb"]["map_header_size"]:
                map_header = rom_data[
                    current_addr : current_addr + self.rom_specs["gb"]["map_header_size"]
                ]
