...
class PlexDiscordPlayer:
    ...
    def __init__(self, bot: commands.Bot, plex_url: str, plex_token: str):
        ...
        if not plex_url or not plex_token:
            ...
            raise ValueError(
                "Plex URL and/or Token is not configured. PlexDiscordPlayer cannot operate."
            )
        ...
        try:
            ...
            logger.error(
                f"Plex authentication failed for PlexDiscordPlayer: Invalid token or credentials for {plex_url}."
            )
            raise PlexConnectionError(f"Invalid Plex credentials for {plex_url}.")
        except RequestsConnectionError as e:
            logger.error(
                f"Plex connection failed for PlexDiscordPlayer: Could not connect to server at {plex_url}. Error: {e}"
            )
            raise PlexConnectionError(f"Could not connect to Plex server at {plex_url}.")
        except RequestsTimeout as e:
            logger.error(
                f"Plex connection timed out for PlexDiscordPlayer while trying to reach {plex_url}. Error: {e}"
            )
            ...
            logger.error(
                f"Unexpected error connecting to Plex for PlexDiscordPlayer ({plex_url}): {e}",
                exc_info=True,
            )
            raise StreamingError(
                f"An unexpected error occurred while connecting to Plex ({plex_url}): {e}"
            )
    ...
    async def create_embed_player(
        ...
        try:
            # Get media from Plex
            try:
                media = self.plex.fetchItem(
                    media_id
                )  # media_id should be int for fetchItem usually
    ...
    async def start_playback(self, ctx: commands.Context):
        ...
        try:
            ...
            logger.error(
                f"Error starting playback for media {state.media_id if state else 'unknown'}: {e}",
                exc_info=True,
            )
    ...
    async def _track_progress(self, ctx: commands.Context):
        ...
        if guild_id not in self.media_sessions or guild_id not in self.current_states:
            logger.warning(
                f"Progress tracking for guild {guild_id} aborted: session or state not found."
            )
            return
        ...
        while state.is_playing:
            # Loop validity check
            if guild_id not in self.media_sessions or not self.current_states.get(guild_id, {}).get(
                "is_playing"
            ):
                logger.info(
                    f"Progress tracking loop for guild {guild_id} terminating: session ended or playback stopped."
                )
                break
            ...
            try:
                ...
                logger.error(
                    f"Error tracking progress for '{state.title}' in guild {guild_id}: {e}",
                    exc_info=True,
                )
                break
    ...
    async def get_clients(self):
        ...
        try:
            ...
            logger.error(f"Plex API error getting clients: {e}")
            raise PlexAPIError(f"Plex API error getting clients: {e}")

    async def get_play_queue(self, client_name: str):
        """Get the play queue for a specific client."""
        try:
            client = self.plex.client(client_name)
            if not client:
                raise MediaNotFoundError(f"Client '{client_name}' not found.")
            # The playQueue is an attribute of the client object in some versions of plexapi
            # For others it is on the session. We will prefer the client's queue.
            if hasattr(client, "playQueue"):
                return client.playQueue
            elif client.session:
                return client.session.playQueue
            return None
        except NotFound:
            raise MediaNotFoundError(f"Client '{client_name}' not found.")
        except PlexApiException as e:
            logger.error(f"Plex API error getting play queue for {client_name}: {e}")
            raise PlexAPIError(f"Plex API error getting play queue for {client_name}: {e}")

    async def skip_next(self, client_name: str):
        ...
        try:
            ...
            logger.error(f"Plex API error skipping next for {client_name}: {e}")
            raise PlexAPIError(f"Plex API error skipping next for {client_name}: {e}")

    async def skip_previous(self, client_name: str):
        ...
        try:
            ...
            logger.error(f"Plex API error skipping previous for {client_name}: {e}")
            raise PlexAPIError(f"Plex API error skipping previous for {client_name}: {e}")
    ...
    async def stop_playback(self, ctx: commands.Context):
        ...
        guild_id = str(ctx.guild.id)
        if guild_id in self.media_sessions:
            session = self.media_sessions.pop(guild_id)
            self.current_states.pop(guild_id, None)
            await session.close()

    async def handle_player_event(self, event: Dict[str, Any]):
        """Handle events from the media player."""
        ...
        if event_type == "media_ended":
            # Handle media ending, e.g., play next in queue
            pass
        elif event_type == "error":
            # Log and handle playback errors
            logger.error(f"Player error in guild {guild_id}: {event.get('message')}")

    def get_current_state(self, guild_id: str) -> Optional[MediaState]:
        """Get the current media state for a guild."""
        return self.current_states.get(guild_id)
