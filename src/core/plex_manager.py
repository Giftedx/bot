...
class PlexManager:
    ...
    def __init__(self) -> None: # New __init__
        ...
        if not self._url or not self._token:
            logger.error("Plex URL or Token not found in configuration. PlexManager cannot operate.")
            raise StreamingError("Plex URL and/or Token is not configured. PlexManager cannot operate.")
    ...
    def _connect(self) -> None:
        ...
        try:
            ...
            logger.error(f"Plex connection failed: Could not connect to server at {self._url}. Error: {e}")
    ...
    def get_stream_url(self, video: Video) -> str:
        ...
        try:
            ...
            logger.error(f"Media not found when trying to get stream URL for video: {video.title}. Error: {e}")
            raise MediaNotFoundError(f"Media not found when trying to get stream URL for: {video.title}")
        except Unauthorized:
            ...
            logger.error(f"Unexpected error getting stream URL for '{video.title}': {e}", exc_info=True)
