...
class AchievementManager:
    ...
    def _init_default_achievements(self) -> None:
        ...
        self.register_achievement(Achievement(
            ...
            exp_reward=1200,
            metadata={
                "attacker_origin": PetOrigin.OSRS,
                "defender_origin": PetOrigin.POKEMON
            }
        ))
        ...
        self.register_achievement(Achievement(
            ...
            exp_reward=1200,
            metadata={
                "attacker_origin": PetOrigin.POKEMON,
                "defender_origin": PetOrigin.OSRS
            }
        ))
        ...
        if event.type == EventType.PET_OBTAINED:
            ...
            if rarity:
                ...
                for achievement in self.achievements.values():
                    if (achievement.category == AchievementCategory.COLLECTION and
                        achievement.metadata.get("rarity") == rarity):
                        progress = tracker.update_progress(
                            achievement.id,
                            tracker.achievements.get(achievement.id, 0) + 1
                        )
                        if progress:
            ...
            if event.data.get("winner_id") == pet_id:
                ...
                for achievement in self.achievements.values():
                    if achievement.category == AchievementCategory.BATTLE:
                        if (achievement.metadata["type"] == "wins" or
                            (achievement.metadata["type"] == "ranked_wins" and 
                             battle_type == "ranked")):
                            progress = tracker.update_progress(
                                achievement.id,
                                tracker.achievements.get(achievement.id, 0) + 1
                ...
                if "opponent_origin" in event.data:
                    for achievement in self.achievements.values():
                        if (achievement.category == AchievementCategory.CROSS_SYSTEM and
                            achievement.metadata["attacker_origin"] == tracker.pet.origin and
                            achievement.metadata["defender_origin"].value == event.data["opponent_origin"]):
                            progress = tracker.update_progress(
                                achievement.id,
                                tracker.achievements.get(achievement.id, 0) + 1
                            )
            ...
            for achievement in self.achievements.values():
                if (achievement.category == AchievementCategory.TRAINING and
                    achievement.metadata["type"] == "level"):
                    progress = tracker.update_progress(
                        achievement.id,
                        new_level
                    )
    ...
    def get_achievements(self, pet: Pet, 
        ...
        for achievement in self.achievements.values():
            ...
            progress = tracker.achievements.get(achievement.id, AchievementProgress(0, achievement.target_value))
    ...
    def get_completion_summary(self, pet: Pet) -> Dict[str, Any]:
        ...
        for category in AchievementCategory:
            ...
            by_category[category.value] = {
                ...
                "percent": (completed_in_category / len(category_achievements) * 100 
