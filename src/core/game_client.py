    ...
    def __post_init__(self):
        ...
        if self.skills is None:
            self.skills = {
                ...
                "runecraft": 1, "construction": 1, "hitpoints": 10,
                "agility": 1, "herblore": 1, "crafting": 1,
                "fletching": 1, "slayer": 1, "hunter": 1,
                "mining": 1, "smithing": 1, "fishing": 1,
                "cooking": 1, "firemaking": 1, "woodcutting": 1,
                "farming": 1, "thieving": 1
            }
        ...
        if self.achievement_diary_progress is None:
            self.achievement_diary_progress = {
                ...
                for region in [
                    ...
                    "karamja", "lumbridge", "morytania", "varrock", "wilderness"
                ]
            }
        ...
        if skill in ("attack", "strength", "defence", "ranged", "magic"):
            for prayer in self.active_prayers:
                if prayer in getattr(PrayerMultipliers, f"{skill.upper()}_PRAYERS", {}):
                    prayer_mult = max(prayer_mult, 
                        PrayerMultipliers.ATTACK_PRAYERS[prayer])
...
class GameClient:
    ...
    async def process_command(self, user_id: str, command: str, args: List[str]) -> str:
        ...
        if command == "train":
            ...
            if action == "master":
                ...
                return f"Current task: {player.slayer_task['amount']} {player.slayer_task['monster']}"
    ...
    async def train_skill(self, player: PlayerState, skill: str) -> str:
        ...
        try:
            ...
            return "\n".join([
                f"You gained {base_xp * level_mult:.1f} {skill} XP.",
                *messages
            ])
        ...
        for patch, timer in player.farming_timers.items():
            ...
            if new_stage > patch_data["stage"]:
                ...
                if new_stage >= stages:
                    ...
                    await self.send_message(player, f"Your {patch_data['seed']} is ready to harvest!")
