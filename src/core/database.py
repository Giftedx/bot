...
class DatabaseManager:
    ...
    def unlock_achievement(self, player_id: str, achievement_type: str, name: str, data: Dict[str, Any]) -> None:
    ...
    def log_chat_message(self, player_id: str, channel_id: str, message: str, score: Optional[float] = None) -> int:
    ...
    def get_roastable_messages(self, min_score: float = 0.5, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    def add_effect(self, player_id: str, effect_type: str, source: str, data: Dict[str, Any], duration_seconds: Optional[int] = None) -> None:
        """Add an active effect for a player"""
        cursor = self.conn.cursor()
        now = datetime.now()
        started_at = now.isoformat()
        expires_at = (now + timedelta(seconds=duration_seconds)).isoformat() if duration_seconds else None
