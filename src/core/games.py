...
class Games(commands.Cog):
    ...
    async def tictactoe(self, ctx, opponent: discord.Member = None):
        ...
        def check(m):
            return (
                m.author in game["players"]
                and m.channel == ctx.channel
                and m.content.isdigit()
                and 1 <= int(m.content) <= 9
            )
        ...
        while True:
            try:
                ...
                # Switch players
                game["current_player"] = (
                    opponent if game["current_player"] == ctx.author else ctx.author
                )
    ...
    async def rps(self, ctx, opponent: discord.Member = None):
        ...
        def check(m):
            return (
                m.author in [ctx.author, opponent]
                and isinstance(m.channel, discord.DMChannel)
                and m.content.lower() in ["rock", "paper", "scissors"]
            )
        ...
        try:
            for player in [ctx.author, opponent]:
                ...
                choice = await self.bot.wait_for(
                    "message", timeout=30.0, check=lambda m: check(m) and m.author == player
                )
            ...
            await ctx.send(
                f"{ctx.author.mention} chose {choices[ctx.author]}\n{opponent.mention} chose {choices[opponent]}"
            )
