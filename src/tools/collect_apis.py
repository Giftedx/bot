...
OSRS_APIS = {
    ...
    "prices": {
        ...
        "endpoints": {"latest": "/latest", "mapping": "/mapping", "5m": "/5m", "1h": "/1h"},
    },
    "ge": {
...
class APICollector:
    ...
    async def collect_api_data(self, api_name: str, api_config: Dict):
        ...
        for endpoint_name, endpoint_path in api_config["endpoints"].items():
            ...
            try:
                headers = {"User-Agent": "OSRS Discord Bot - Data Collection"}
                ...
                logger.error(f"Error fetching {endpoint_name} from {api_name}: {e}")
