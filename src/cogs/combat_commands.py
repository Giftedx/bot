...
class CombatCommands(commands.Cog):
    ...
    def create_player_entity(self, member: discord.Member) -> CombatEntity:
        ...
        stats = CombatStats(
            attack=1, strength=1, defence=1, ranged=1, magic=1, prayer=1, hitpoints=10
        )
    ...
    async def view_combat_stats(self, interaction: discord.Interaction):
        ...
        if not stats:
            await interaction.response.send_message(
                "You don't have any combat stats yet!", ephemeral=True
            )
            return

        embed = discord.Embed(
            title=f"{interaction.user.name}'s Combat Stats", color=discord.Color.red()
        )
    ...
    async def change_attack_style(self, interaction: discord.Interaction, style: CombatStyle):
        ...
        if not success:
            await interaction.response.send_message(
                "Failed to change attack style. Please try again.", ephemeral=True
            )
            return
    ...
    async def toggle_prayer(self, interaction: discord.Interaction, prayer: str, enabled: bool):
        """Toggle prayer effects on/off"""
        # Get player's prayer points
        prayer_points = await self.get_prayer_points(interaction.user.id)
        if prayer_points <= 0:
            await interaction.response.send_message(
                "You have no prayer points! Recharge at an altar.", ephemeral=True
            )
            return
    ...
    async def special_attack(self, interaction: discord.Interaction):
        ...
        if energy < 50:  # Most special attacks use 50% energy
            await interaction.response.send_message(
                f"Not enough special attack energy! Current energy: {energy}%", ephemeral=True
            )
            return
        ...
        if not weapon or not weapon.get("special_attack"):
            await interaction.response.send_message(
                "Your current weapon has no special attack!", ephemeral=True
            )
            return
        ...
        if not success:
            await interaction.response.send_message(
                "Failed to use special attack. Please try again.", ephemeral=True
            )
            return
        ...
        embed.add_field(name="Energy Remaining", value=f"{energy - 50}%")
        await interaction.response.send_message(embed=embed)
    ...
    async def restore_stats(self, interaction: discord.Interaction):
        ...
        if not success:
            await interaction.response.send_message(
                "Failed to restore stats. Please try again.", ephemeral=True
            )
            return
    ...
    async def eat_food(self, interaction: discord.Interaction, food: str):
        """Eat food to restore hitpoints"""
        # Check if player has the food
        has_food = await self.check_inventory(interaction.user.id, food)
        if not has_food:
            await interaction.response.send_message(f"You don't have any {food}!", ephemeral=True)
            return

        # Get food healing amount
        healing = await self.get_food_healing(food)
        if healing is None:
            await interaction.response.send_message(f"Invalid food item: {food}", ephemeral=True)
            return
        ...
        if not success:
            await interaction.response.send_message(
                "Failed to eat food. Please try again.", ephemeral=True
            )
            return

        embed = discord.Embed(
            title="Ate Food", description=f"You eat the {food}.", color=discord.Color.green()
        )
        embed.add_field(name="Healed", value=f"+{healing} HP")
        embed.add_field(name="Current HP", value=str(new_hp))
        await interaction.response.send_message(embed=embed)

    @app_commands.command(name="potion", description="Drink a potion")
    async def drink_potion(self, interaction: discord.Interaction, potion: str):
        ...
        if not has_potion:
            await interaction.response.send_message(f"You don't have any {potion}!", ephemeral=True)
            return

        # Get potion effects
        effects = await self.get_potion_effects(potion)
        if not effects:
            await interaction.response.send_message(f"Invalid potion: {potion}", ephemeral=True)
            return
        ...
        if not success:
            await interaction.response.send_message(
                "Failed to drink potion. Please try again.", ephemeral=True
            )
            return
    ...
    async def monsters_command(
        ...
        if not monsters:
            await interaction.response.send_message(
                f"No monsters found between combat levels {min_level} and {max_level}."
            )
            return

        # Create embed
        embed = discord.Embed(
            title=f"Monsters (Combat {min_level}-{max_level})", color=discord.Color.blue()
        )
        ...
        for level in sorted(by_level.keys()):
            ...
            embed.add_field(name=f"Level {level}", value="\n".join(names), inline=True)
    ...
    async def fight_command(self, interaction: discord.Interaction, monster_name: str):
        ...
        embed = discord.Embed(title=f"Fight: {monster['name']}", color=discord.Color.red())
                ...
                if "boss_info" in metadata["additional_info"]:
                    ...
                    embed.add_field(
                        ...
                        value="\n".join(
                            [
                                f"• {stat.title()}: {level}"
                                for stat, level in recommended_stats.items()
                            ]
                        ),
                        inline=True,
                    )
    ...
    async def attack_command(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"Combat with {monster['name']}", color=discord.Color.red())
        ...
        if result["player_damage"] > 0:
            ...
            embed.add_field(
                name="Your Attack", value=f"You missed the {monster['name']}!", inline=False
            )
        ...
        if result["monster_damage"] > 0:
            ...
            embed.add_field(
                name="Monster Attack", value=f"The {monster['name']} missed you!", inline=False
            )
        ...
        if result["combat_ended"]:
            if result["victory"]:
                embed.add_field(
                    name="Victory!", value=f"You defeated the {monster['name']}!", inline=False
                )
                ...
                if result["drops"]:
                    ...
                    embed.add_field(
                        name="Drops Received", value="\n".join(drops_text), inline=False
                    )
    ...
    async def flee_command(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="Escape Attempt", color=discord.Color.yellow())
    ...
    async def apply_potion(
        self, player_id: int, effects: Dict[str, int]
