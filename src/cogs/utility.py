...
class UtilityCog(BaseCog):
    ...
    async def ping(self, ctx: commands.Context) -> None:
        ...
        embed.add_field(name="Bot Latency", value=f"{(end_time - start_time) * 1000:.2f}ms")
    ...
    async def bot_info(self, ctx: commands.Context) -> None:
        ...
        # General info
        embed.add_field(name="Bot Version", value=self.config.get_config("version", "1.0.0"))
    ...
    async def list_roles(
        ...
        await self.send_paginated_embed(
            ctx, title, role_list, items_per_page=10, color=discord.Color.blue()
        )

    @commands.hybrid_command(name="remind")
    async def remind(self, ctx: commands.Context, duration: int, *, reminder: str) -> None:
        """Set a reminder.
