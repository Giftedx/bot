...
class WatchCommands(commands.Cog):
    ...
    async def watch_start(self, ctx, *, content: str):
        ...
        embed.add_field(
            name="Status",
            value="Waiting for members",
            inline=True
        )
        
        embed.add_field(
            name="Join Command",
            value=f"`!watch join {party_id}`",
            inline=False
        )
    ...
    async def watch_join(self, ctx, party_id: str):
        ...
        embed.add_field(
            name="Host",
            value=self.bot.get_user(int(party['host'])).display_name,
            inline=True
        )
        
        embed.add_field(
            name="Members",
            value=len(party['members']),
            inline=True
        )
    ...
    async def watch_start_watching(self, ctx, party_id: str):
        ...
        for member_id in party['members']:
            self.bot.db.add_watch_record(member_id, {
                'id': party_id,
                'type': 'group_watch',
                'title': party['content'],
    ...
    async def watch_pause(self, ctx):
        ...
        party = next(
            (p for p in self.active_parties.values() if player_id in p['members']),
            None
        )
    ...
    async def watch_status(self, ctx):
        ...
        if party['status'] == 'watching':
            ...
            embed.add_field(
                name="State",
                value="Paused" if party['paused'] else "Playing",
                inline=True
            )
    ...
    async def watch_leave(self, ctx):
        ...
        if player_id == party['host']:
            # Find party ID
            party_id = next(
                pid for pid, p in self.active_parties.items() if p == party
            )
    ...
    async def cleanup_party(self, party_id: str):
        ...
        try:
            ...
            timeout = self.bot.config.get_config(
                'watch_parties.auto_end_minutes',
                360  # 6 hours default
            )
