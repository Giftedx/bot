...
class PokemonTournament(commands.Cog):
    ...
    async def create_tournament(self, ctx):
        ...
        try:
            ...
            # Get tournament type
            type_list = "\n".join(
                f"• {t}: {data['description']}" for t, data in self.tournament_types.items()
            )
    ...
    async def tournament_info(self, ctx, tournament_id: int):
        ...
        tournament = await self.bot.db.fetchrow(
            "SELECT * FROM pokemon_tournaments WHERE id = $1", tournament_id
        )
        ...
        embed = discord.Embed(title=f"Tournament: {tournament['name']}", color=discord.Color.blue())

        embed.add_field(name="ID", value=str(tournament["id"]), inline=True)
        embed.add_field(
            name="Type", value=tournament["tournament_type"].replace("_", " ").title(), inline=True
        )
        embed.add_field(
        ...
        embed.add_field(
            name="Level Range",
            value=f"{tournament['min_level']}-{tournament['max_level']}",
            inline=True,
        )
    ...
    async def join_tournament(self, ctx, tournament_id: int):
        ...
        for i, p in enumerate(pokemon[:12], 1):  # Show up to 12 Pokemon
            ...
            pokemon_list.append(f"{i}. Level {p['level']} {p['name'].title()} ({types_str})")
        ...
        if rules:
            ...
            embed.add_field(name="Tournament Rules", value="\n".join(rules_text), inline=False)
        ...
        try:
            ...
            if (
                tournament["tournament_type"] == "level_cap"
                and rules.get("scale_to_cap")
                and "level_cap" in rules
            ):
    ...
    async def leave_tournament(self, ctx, tournament_id: int):
        ...
        await ctx.send(
            "You've successfully left the tournament and received a refund of the entry fee!"
        )
    ...
    async def start_tournament(self, ctx, tournament_id: int):
        ...
        # Get participants
        participants = await self.bot.db.fetch(
            "SELECT user_id FROM tournament_participants WHERE tournament_id = $1", tournament_id
        )
        ...
        for match in matches:
            if match["player2_id"]:
                ...
                match_list.append(
                    f"Match {match['match_number']}: " f"{match['player1_name']} receives a bye"
                )

        embed.add_field(name="First Round Matches", value="\n".join(match_list), inline=False)
    ...
    async def view_bracket(self, ctx, tournament_id: int):
        ...
        embed = discord.Embed(
            title=f"Tournament Bracket: {tournament['name']}", color=discord.Color.blue()
        )
        ...
        for match in matches:
            if match["round"] != current_round:
                if match_list:
                    embed.add_field(
                        name=f"Round {current_round}", value="\n".join(match_list), inline=False
                    )
            ...
            if match["player2_id"]:
                ...
                match_list.append(
                    f"Match {match['match_number']}: {status} " f"{match['player1_name']} (bye)"
                )
    ...
    async def view_matches(self, ctx):
        ...
        embed = discord.Embed(title="Your Tournament Matches", color=discord.Color.blue())
    ...
    async def tournament_battle(self, ctx, tournament_id: int, match_number: int):
        ...
        # Get opponent
        opponent_id = (
            match["player2_id"] if ctx.author.id == match["player1_id"] else match["player1_id"]
        )
        ...
        player_pokemon = await self.bot.db.fetch(
            ...
            match["player1_pokemon"]
            if ctx.author.id == match["player1_id"]
            else match["player2_pokemon"],
        )
        opponent_pokemon = await self.bot.db.fetch(
            "SELECT * FROM pokemon WHERE id = ANY($1)",
            match["player2_pokemon"]
            if ctx.author.id == match["player1_id"]
            else match["player1_pokemon"],
        )
    ...
    async def handle_tournament_victory(self, ctx, winner_id):
        ...
        # Get tournament info
        tournament = await self.bot.db.fetchrow(
            "SELECT * FROM pokemon_tournaments WHERE id = $1", battle["tournament_id"]
        )
        ...
        if remaining_matches == 0:
            ...
            embed.add_field(name="Prize", value=f"{tournament['prize_pool']} coins", inline=False)
            await ctx.send(embed=embed)
        else:
    ...
    async def generate_next_round_matches(self, ctx, tournament_id: int):
        ...
        if len(winners) % 2 == 0 and all(w["winner_id"] for w in winners):
            ...
            # Announce next round
            embed = discord.Embed(title=f"Round {next_round} Matches", color=discord.Color.blue())
    ...
    async def create_tournament_battle_embed(self, ctx, battle: Dict) -> discord.Embed:
        ...
        embed = discord.Embed(title="Tournament Battle", color=discord.Color.blue())
        ...
        for trainer_id, data in battle["trainers"].items():
            if data:  # Skip if trainer hasn't chosen Pokemon yet
                ...
                for i, p in enumerate(data["pokemon"]):
                    status = (
                        "💀"
                        if i in data["fainted_pokemon"]
                        else "⚔️"
                        if i == data["current_pokemon"]
                        else "⏳"
                    )
    ...
    async def tournament_history(self, ctx, user: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"Tournament History - {target_user.display_name}", color=discord.Color.blue()
        )
    ...
    async def tournament_rules(self, ctx, tournament_id: int = None):
        ...
        if tournament_id:
            ...
            embed.add_field(
                name="Prize Pool", value=f"{tournament['prize_pool']} coins", inline=True
            )
            ...
            if tournament["rules"]:
                ...
                for rule, value in rules.items():
                    embed.add_field(
                        name=rule.replace("_", " ").title(), value=str(value), inline=False
                    )

        else:
            # Show all tournament types and their rules
            embed = discord.Embed(title="Tournament Types and Rules", color=discord.Color.blue())

            for t_type, data in self.tournament_types.items():
                rules_text = (
                    "\n".join(f"• {k}: {v}" for k, v in data["rules"].items())
                    if data["rules"]
