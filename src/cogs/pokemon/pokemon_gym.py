...
class PokemonGym(commands.Cog):
    def __init__(self, bot):
        ...
        self.gym_rematches = {
            gym_id: self._create_rematch_data(gym_data) for gym_id, gym_data in self.gyms.items()
        }

        # Elite Four rematch data
        self.elite_four_rematches = {
            member_id: self._create_rematch_data(member_data, elite=True)
    ...
    async def gym_info(self, ctx, gym_id: str):
        ...
        if cooldown > ctx.message.created_at.timestamp():
            ...
            embed.add_field(name="Status", value="Ready for challenge!", inline=False)
    ...
    async def challenge_gym(self, ctx, gym_id: str):
        ...
        if cooldown > ctx.message.created_at.timestamp():
            ...
            return await ctx.send(
                f"You must wait {hours_left} hours before challenging this gym again!"
            )
        ...
        user_pokemon = await self.bot.db.fetch(
            "SELECT * FROM pokemon WHERE user_id = $1 ORDER BY level DESC", ctx.author.id
        )
        ...
        def check(m):
            return (
                ...
                and 1 <= int(m.content) <= len(pokemon_list)
            )
    ...
    async def gym_attack(self, ctx, move_name: str):
        ...
        try:
            ...
            if battle["leader"]["current_hp"] <= 0:
                ...
                await ctx.send(f"{gym_data['leader']}: Go, {new_pokemon['name'].title()}!")
    ...
    async def view_badges(self, ctx):
        ...
        for badge in badges:
            ...
            gym_data = next((gym for gym in self.gyms.values() if gym["badge"] == badge), None)
            if gym_data:
                embed.add_field(
                    name=badge,
    ...
    async def get_user_badges(self, user_id: int) -> List[str]:
        ...
        badges = await self.bot.db.fetch("SELECT badge FROM gym_badges WHERE user_id = $1", user_id)
    ...
    async def create_gym_battle_embed(self, ctx, battle: Dict) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=f"Gym Battle vs {gym_data['leader']}", color=discord.Color.red()
        )

        # Trainer's Pokemon
        trainer_pokemon = battle["trainer"]["pokemon"]
        trainer_hp_percent = int(
            battle["trainer"]["current_hp"] / self.calculate_hp(trainer_pokemon) * 100
        )
        ...
        # Leader's Pokemon
        leader_hp_percent = int(
            battle["leader"]["current_hp"] / self.calculate_hp(leader_pokemon) * 100
        )
    ...
    async def challenge_elite(self, ctx):
        ...
        user_pokemon = await self.bot.db.fetch(
            "SELECT * FROM pokemon WHERE user_id = $1 ORDER BY level DESC LIMIT 6", ctx.author.id
        )
    ...
    async def start_elite_challenge(self, ctx, user_pokemon):
        ...
        for member_id, member_data in self.elite_four.items():
            ...
            if not await self.get_elite_progress(ctx.author.id, member_id):
                return await ctx.send(
                    "You were defeated by the Elite Four! Try again after training more!"
                )
    ...
    async def handle_elite_victory(self, ctx):
        ...
        await self.bot.db.execute(
            "UPDATE users SET currency = currency + $1 WHERE id = $2", total_reward, ctx.author.id
        )
    ...
    async def elite_status(self, ctx):
        ...
        embed = discord.Embed(title="Elite Four Progress", color=discord.Color.purple())
        ...
        for member_id, member_data in self.elite_four.items():
            ...
            embed.add_field(name=member_data["name"], value=status, inline=False)
    ...
    async def champion_info(self, ctx):
        ...
        pokemon_list = "\n".join(
            f"• Level {p['level']} {p['name'].title()}" for p in self.champion["pokemon"]
        )

        embed.add_field(name="Pokemon Team", value=pokemon_list, inline=False)
        embed.add_field(name="Reward", value=f"${self.champion['reward_money']:,}", inline=False)
        embed.add_field(
            name="Requirements",
    ...
    async def challenge_champion(self, ctx):
        ...
        is_champion = await self.bot.db.fetchrow(
            "SELECT completed_at FROM elite_four_champions WHERE user_id = $1", ctx.author.id
        )
        if not is_champion:
    ...
    async def elite_rematch(self, ctx):
        ...
        if not is_champion:
            return await ctx.send(
                "You must defeat the Elite Four normally before requesting a rematch!"
            )
    ...
    async def champion_history(self, ctx):
        ...
        embed = discord.Embed(title="Champion Battle History", color=discord.Color.gold())
