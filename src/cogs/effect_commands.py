...
class EffectCommands(commands.Cog):
    ...
    async def effect_info(self, ctx, effect_id: int):
        ...
        embed.add_field(
            name="Status",
            value="Active" if effect['active'] else "Expired",
            inline=True
        )
    ...
    @commands.has_permissions(manage_messages=True)
    async def effect_add(self, ctx, target: discord.Member, effect_type: str, duration: Optional[str] = None):
        ...
        effect_id = self.bot.db.add_effect(
            ...
            'admin',
            {
                'description': f"Added by {ctx.author.display_name}",
                'stats': {
                    'admin_buff': 1
                }
            },
