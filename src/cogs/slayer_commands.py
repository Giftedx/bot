...
class SlayerCommands(commands.Cog):
    ...
    async def get_task(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title="Slayer Task", color=discord.Color.blue())
        ...
        if task.experience_gained > 0:
            embed.add_field(name="XP Gained", value=f"{task.experience_gained:,.1f}", inline=True)
    ...
    async def get_master_task(self, ctx: commands.Context, master_name: str):
        ...
        # Assign new task
        task = self.slayer_system.assign_task(master_name, stats.slayer, blocked_tasks)
        ...
        if not task:
            ...
            if master and stats.slayer < master.level_req:
                await ctx.send(f"You need level {master.level_req} Slayer to use {master.name}.")
            else:
                await ctx.send(
    ...
    async def list_masters(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title="Slayer Masters", color=discord.Color.blue())
        ...
        for master in self.slayer_system.MASTERS.values():
            if stats.slayer >= master.level_req:
                available.append(f"• {master.name} (Level {master.level_req})")
            else:
                locked.append(f"• {master.name} (Level {master.level_req})")

        if available:
            embed.add_field(name="Available Masters", value="\n".join(available), inline=False)

        if locked:
            embed.add_field(name="Locked Masters", value="\n".join(locked), inline=False)
    ...
    async def check_points(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title="Slayer Points", color=discord.Color.blue())

        embed.add_field(name="Points", value=str(self.slayer_system.points), inline=True)

        embed.add_field(name="Task Streak", value=str(self.slayer_system.task_streak), inline=True)

        embed.add_field(name="Total Tasks", value=str(self.slayer_system.total_tasks), inline=True)
    ...
    async def list_rewards(self, ctx: commands.Context):
        ...
        if unlockables:
            embed.add_field(name="Unlockable Rewards", value="\n".join(unlockables), inline=False)
        ...
        embed.add_field(name="Point Costs", value="\n".join(costs), inline=False)
    ...
    async def cancel_task(self, ctx: commands.Context):
        ...
        if self.slayer_system.cancel_task():
            ...
            await ctx.send("You don't have a task to cancel, or you don't have enough points!")
    ...
    async def block_task(self, ctx: commands.Context):
        ...
        if blocked:
            ...
            await ctx.send(f"{monster_name} tasks have been blocked for 100 points.")
        else:
            await ctx.send("You don't have a task to block, or you don't have enough points!")
    ...
    async def unlock_reward(self, ctx: commands.Context, *, reward_name: str):
        ...
        if self.slayer_system.unlock_reward(reward_name.lower().replace(" ", "_")):
            ...
            await ctx.send("Invalid reward, already unlocked, or not enough points!")
