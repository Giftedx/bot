...
class EventCommands(commands.Cog):
    ...
    async def event_list(self, ctx):
        ...
        # Create embed
        embed = discord.Embed(
            title="Active Events", color=discord.Color.blue(), timestamp=datetime.now()
        )
        ...
        for event_type, type_events in events_by_type.items():
            ...
            embed.add_field(name=event_type.title(), value="\n".join(event_list), inline=False)
    ...
    async def event_info(self, ctx, event_id: int):
        ...
        embed.add_field(name="Status", value=event["status"].title(), inline=True)

        # Add duration
        started = datetime.fromisoformat(event["started_at"])
        embed.add_field(name="Started", value=started.strftime("%Y-%m-%d %H:%M:%S"), inline=True)
    ...
    async def event_start(self, ctx, event_type: str, duration: str, *, name: str):
        ...
        try:
            ...
            await ctx.send(
                "Invalid duration format! Use a number followed by h/m/s " "(e.g. 2h for 2 hours)"
            )
            return
        ...
        event_id = self.bot.db.start_event(
            event_type,
            {
                ...
                "effects": {"xp_multiplier": 1.5, "drop_rate": 1.25},
            },
        )
        ...
        # Add default effects
        embed.add_field(
            name="Effects", value=("• XP Multiplier: +50%\n" "• Drop Rate: +25%"), inline=False
        )
