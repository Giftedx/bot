...
class OSRSDataManager:
    ...
    async def load_quests(self):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get('https://raw.githubusercontent.com/oldschoolgg/oldschooljs/master/src/data/quests.json') as resp:
                    if resp.status == 200:
    ...
    async def search_items(self, query: str) -> List[Dict]:
        ...
        query = query.lower()
        return [item for item in self.items_cache.values() 
    ...
    async def get_ge_price(self, item_id: int) -> Optional[Dict]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f'https://prices.runescape.wiki/api/v1/osrs/latest?id={item_id}') as resp:
                    if resp.status == 200:
    ...
    async def get_quest_info(self, quest_name: str) -> Optional[Dict]:
        ...
        return next((quest for quest in self.quests_cache 
                    if quest['name'].lower() == quest_name), None)
