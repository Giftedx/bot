...
from .locations import (
    LocationManager, Location, Connection,
    TravelMethod, RegionType
)
...
class WebWalking:
        ...
        while frontier:
            ...
            for next_coord in self.web_nodes[current].connections:
                new_cost = cost_so_far[current] + self.calculate_distance(
                    self.web_nodes[current].tile,
                    self.web_nodes[next_coord].tile
                )
    ...
    def is_tile_walkable(self, tile: Tile) -> bool:
        ...
        if not (0 <= tile.x < len(self.movement.current_area.tiles[0]) and 
                0 <= tile.y < len(self.movement.current_area.tiles)):
            return False
    ...
    def get_minimap_destination(
        ...
        if state.current_path.location == current_location:
            if state.current_path.next_node:
                next_loc = self.locations.get_location(
                    state.current_path.next_node.location
                )
                if next_loc:
