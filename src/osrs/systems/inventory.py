...
class InventoryManager:
    ...
    def __init__(self):
        ...
        self.player_inventories: Dict[
            str, Dict[str, int]
        ] = {}  # player_id -> item_name -> quantity
        self.player_equipment: Dict[
            str, Dict[EquipmentSlot, str]
        ] = {}  # player_id -> slot -> item_name
    ...
    def _load_items(self) -> Dict[str, Item]:
        ...
        return {
            "Bronze sword": Item(
                ...
                stats=ItemStats(attack_stab=4, attack_slash=3, attack_crush=2, melee_strength=5),
            ),
            "Rune platebody": Item(
                name="Rune platebody",
                type=ItemType.ARMOR,
                ...
                requirements=ItemRequirements(skills={"defence": 40}, quests=["Dragon Slayer"]),
            ),
            "Coins": Item(
                name="Coins", type=ItemType.CURRENCY, tradeable=True, stackable=True, value=1
            ),
            # Add more items...
        }

    def get_inventory(self, player_id: str) -> Dict[str, int]:
        """Get player's inventory"""
        if player_id not in self.player_inventories:
            self.player_inventories[player_id] = {}
        return self.player_inventories[player_id]

    def get_equipment(self, player_id: str) -> Dict[EquipmentSlot, str]:
        """Get player's equipped items"""
        if player_id not in self.player_equipment:
            self.player_equipment[player_id] = {}
        return self.player_equipment[player_id]

    def get_bank(self, player_id: str) -> Dict[str, int]:
        """Get player's bank"""
    ...
    def add_item(
        self, player_id: str, item_name: str, quantity: int, to_bank: bool = False
    ) -> bool:
        ...
        if not item.stackable and not to_bank:
            ...
            current_slots = sum(
                qty for name, qty in target.items() if not self.items[name].stackable
            )
    ...
    def remove_item(
        self, player_id: str, item_name: str, quantity: int, from_bank: bool = False
    ) -> bool:
    ...
    def equip_item(self, player_id: str, item_name: str) -> Optional[str]:
    ...
    def get_equipment_stats(self, player_id: str) -> ItemStats:
