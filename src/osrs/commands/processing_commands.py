...
class ProcessingCommands(commands.Cog):
    ...
    async def cook(self, interaction: discord.Interaction, fish: str):
        ...
        # Attempt to cook
        success, xp = self.processing_manager.attempt_process(item_type, player.skills, has_ingredients)
        ...
        if success:
            ...
            await interaction.response.send_message(
                f"You accidentally burn the {fish.lower()}.",
                ephemeral=True
            )
    
    @app_commands.command(name="smelt")
    @app_commands.describe(bar="The type of bar to smelt (bronze, iron, steel, mithril, adamant, rune)")
