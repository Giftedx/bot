...
class EventManager:
    ...
    def register_default_handlers(self) -> None:
        ...
        self.register_handler(
            ...
            self._handle_quest_complete
        )
        
        # Achievement handlers
        self.register_handler(
            EventType.ACHIEVEMENT_COMPLETE,
            self._handle_achievement_complete
        )
        ...
        self.register_handler(
            ...
            self._handle_combat_death
        )
        
        # Item handlers
        self.register_handler(
            EventType.ITEM_ACQUIRED,
            self._handle_item_acquired
        )
        
        # Location handlers
        self.register_handler(
            EventType.LOCATION_REACHED,
            self._handle_location_reached
        )
        
        # Minigame handlers
        self.register_handler(
            EventType.MINIGAME_COMPLETE,
            self._handle_minigame_complete
        )
        
        # Title handlers
        self.register_handler(
            EventType.TITLE_UNLOCKED,
            self._handle_title_unlocked
        )
        ...
        self.register_handler(
            ...
            self._handle_boost_applied
        )
        self.register_handler(
            EventType.BOOST_EXPIRED,
            self._handle_boost_expired
        )
        
    def dispatch_event(self, event: GameEvent) -> None:
        """Dispatch an event to its handlers."""
        logger.info(
            f"Dispatching event {event.event_type} for player {event.player_id}"
        )
    ...
    def _handle_combat_death(self, event: GameEvent) -> None:
        ...
        self.game_state.record_combat_stats(
            event.player_id,
            deaths=1,
            damage_taken=event.data.get('damage_taken', 0)
        )
    ...
    def _handle_item_acquired(self, event: GameEvent) -> None:
        ...
        if event.data.get('add_to_collection', False):
            self.game_state.update_collection_log(
                event.player_id,
                event.data.get('category', 'Miscellaneous'),
                item_id,
                quantity
            )
    ...
    def _handle_location_reached(self, event: GameEvent) -> None:
        ...
        location = WorldPoint(
            event.data['x'],
            event.data['y'],
            event.data.get('plane', 0)
        )
        
        # Update player location
        self.game_state.update_player_location(
            event.player_id,
            location
        )
    ...
    def _handle_title_unlocked(self, event: GameEvent) -> None:
        ...
        self.game_state.add_player_title(
            event.player_id,
            event.data['title'],
            event.data.get('make_active', False)
        )
    ...
    def _handle_relationship_formed(self, event: GameEvent) -> None:
        ...
        self.game_state.add_relationship(
            event.player_id,
            event.data['related_player_id'],
            event.data['relationship_type']
        )
