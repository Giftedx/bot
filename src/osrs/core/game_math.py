...
def calculate_max_hit(
    ...
    effective_str = floor(
        ((strength_level + style_bonus + 8) * prayer_bonus) * other_bonus
    )
...
def calculate_hit_chance(
    ...
    effective_atk = floor(
        ((attack_level + style_bonus + 8) * prayer_bonus) * other_bonus
    )
