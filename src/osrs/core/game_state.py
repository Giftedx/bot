...
class GameState:
    ...
    def start_training_session(
        self, player_id: int, skill_name: str, location: WorldPoint
    ) -> Optional[int]:
        """Start a new training session."""
        player = next((p for p in self.active_players.values() if p["id"] == player_id), None)
        if not player:
            return None
    ...
    def end_training_session(self, session_id: int, xp_gained: int) -> bool:
        ...
        with self.db.conn:
            ...
            cursor.execute(
                ...
                WHERE id = ?
                """,
                (datetime.utcnow(), new_level, xp_gained, session_id),
            )
            ...
            cursor.execute(
                ...
                WHERE player_id = ? AND skill_type = ?
                """,
                (xp_gained, new_level, session["player_id"], session["skill_name"]),
            )
    ...
    def record_transaction(
        ...
        with self.db.conn:
            ...
            cursor.execute(
                ...
                """,
                (player_id, item_id, quantity, price_per_item, transaction_type),
            )

    def update_collection_log(
        self, player_id: int, category: str, item_id: str, count: int = 1
    ) -> None:
    ...
    def update_minigame_score(self, player_id: int, minigame: str, score: int) -> None:
        ...
        with self.db.conn:
            ...
            cursor.execute(
                ...
                """,
                (player_id, minigame, score, score, score, score, score),
            )

    def add_player_title(self, player_id: int, title: str, make_active: bool = False) -> None:
    ...
    def add_relationship(
        self, player_id: int, related_player_id: int, relationship_type: str
    ) -> None:
    ...
    def add_cross_system_boost(
        ...
        expires_at = datetime.utcnow().timestamp() + duration_seconds if duration_seconds else None
        ...
        with self.db.conn:
            ...
            cursor.execute(
                ...
                """,
                (player_id, source_type, target_type, boost_value, expires_at),
            )
    ...
    def _calculate_level(self, xp: int) -> int:
        ...
        for lvl in range(2, 100):
            xp_table.append(int(xp_table[-1] + lvl + 300 * (2 ** (lvl / 7.0))))
            if xp >= xp_table[-1]:
                level = lvl
            else:
                break
