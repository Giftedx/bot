...
class MovementSystem:
    ...
    def __init__(self, game_tick):
        ...
        self.game_tick.register_task("movement_update", self._movement_tick, TickPriority.MOVEMENT)
    ...
    def update_energy(
        self, current_energy: float, agility_level: int, running: bool, ticks: int = 1
    ) -> float:
    ...
    def calculate_path(
        ...
        def get_neighbors(pos: Position) -> List[Position]:
            ...
            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:  # OSRS uses 4-directional movement
                ...
                if (
                    0 <= new_x < len(collision_map[0])
                    and 0 <= new_y < len(collision_map)
                    and not collision_map[new_y][new_x]
                ):
    ...
    def calculate_travel_time(
        self, path: List[Position], agility_level: int, starting_energy: float
    ) -> Tuple[float, float]:
        ...
        while current_pos < len(path) - 1:
            ...
            current_energy = self.update_energy(current_energy, agility_level, running, ticks)
            total_ticks += ticks
