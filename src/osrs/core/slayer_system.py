...
class SlayerMaster:
    def __init__(
        ...
        self.weights = {monster: weight / total_weight for monster, weight in assignments.items()}
...
class SlayerSystem:
    ...
    MASTERS = {
        "turael": SlayerMaster(
            ...
            level_req=1,
            task_range=(15, 25),
            assignments={"goblin": 6, "giant_rat": 6, "hill_giant": 4, "lesser_demon": 2},
        ),
    ...
    REWARDS = {
        ...
        "unlocks": {
            ...
            "slayer_ring": {"points": 300, "description": "Ability to craft slayer rings"},
            "herb_sack": {"points": 750, "description": "Store herbs in a special sack"},
            "rune_pouch": {"points": 1200, "description": "Store runes in a special pouch"},
        },
    }
    ...
    def get_available_masters(self, slayer_level: int) -> List[SlayerMaster]:
        ...
        return [master for master in self.MASTERS.values() if master.level_req <= slayer_level]

    def assign_task(
        self, master_name: str, slayer_level: int, blocked_tasks: List[str] = None
    ) -> Optional[SlayerTask]:
        ...
        # Choose monster
        monster_id = random.choices(list(available.keys()), weights=list(available.values()))[0]
