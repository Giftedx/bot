...
class GearSetup:
    ...
    def equip(self, item: GearItem) -> List[GearItem]:
        ...
        if (
            ...
            and self._slots[EquipmentSlot.WEAPON]
            and self._slots[EquipmentSlot.WEAPON].two_handed
        ):
    ...
    def equipped_items(self) -> Set[Union[str, int]]:
        ...
        return {item.id for item in self._slots.values() if item is not None}

    def meets_requirements(self, skills: Dict[SkillType, int], completed_quests: Set[str]) -> bool:
...
class GearBank:
    ...
    def __init__(self, gear: Dict[str, GearSetup], bank: Bank, skills: Dict[SkillType, int]):
        """Initialize with gear setups, bank and skills."""
        self.gear = gear
        self.bank = bank
        self.skills = skills

    def equip(self, setup: str, item: GearItem, auto_unequip_to_bank: bool = True) -> bool:
        """
    ...
    def unequip(self, setup: str, slot: EquipmentSlot, to_bank: bool = True) -> bool:
        """
    ...
    def clear_setup(self, setup: str, to_bank: bool = True) -> bool:
        """
