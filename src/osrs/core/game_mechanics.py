...
class GameMechanics:
    ...
    def calculate_damage_reduction(
        defence_level: int, equipment_bonus: int, prayer_bonus: float = 1.0
    ) -> float:
    ...
    def calculate_prayer_drain(
        points: int, prayer_bonus: int, active_prayers: Dict[str, float]
    ) -> float:
    ...
    @staticmethod
    def calculate_skill_xp(base_xp: float, level: int, bonus_multiplier: float = 1.0) -> int:
    ...
    def calculate_drop_chance(
        base_chance: float, luck_bonus: float = 0, ring_of_wealth: bool = False
    ) -> float:
    ...
    def calculate_resource_depletion(
        base_chance: float, skill_level: int, tool_bonus: float = 0
    ) -> float:
    ...
    def calculate_burn_chance(
        cooking_level: int, food_level: int, cooking_gauntlets: bool = False
    ) -> float:
    ...
    def calculate_agility_success(
        agility_level: int, obstacle_level: int, graceful_bonus: bool = False
    ) -> float:
    ...
    def calculate_thieving_success(
        thieving_level: int, target_level: int, ardougne_bonus: bool = False
    ) -> float:
    ...
    @staticmethod
    def calculate_mining_speed(mining_level: int, rock_level: int, pickaxe_bonus: float) -> float:
    ...
    def calculate_woodcutting_speed(
        woodcutting_level: int, tree_level: int, axe_bonus: float
    ) -> float:
    ...
    @staticmethod
    def calculate_fishing_speed(fishing_level: int, fish_level: int, tool_bonus: float) -> float:
