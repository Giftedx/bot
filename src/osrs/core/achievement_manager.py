...
class AchievementManager:
    ...
    def __init__(self, game_tick: GameTick):
        ...
        self.player_progress: Dict[
            int, Dict[str, AchievementProgress]
        ] = {}  # player_id: {achievement_id: progress}

        # Register achievement tick task
        self.game_tick.register_task(
            "achievement_update", self._achievement_tick, TickPriority.WORLD
        )
    ...
    def get_player_progress(
        self, player_id: int, achievement_id: str
    ...
    def update_progress(
        self, player_id: int, requirement_type: str, requirement_id: str, value: int
    ):
    ...
    def _complete_achievement(self, player_id: int, achievement_id: str):
    ...
    def _grant_reward(self, player_id: int, reward: AchievementReward):
    ...
    def get_completed_achievements(
        self, player_id: int, category: Optional[AchievementCategory] = None
    ) -> List[Achievement]:
    ...
    def get_series_progress(self, player_id: int, series: str) -> Tuple[int, int]:
        ...
        completed = sum(
            1
            for achievement in series_achievements
            if self.player_progress.get(player_id, {})
            .get(achievement.id, AchievementProgress(achievement))
            .completed
        )
