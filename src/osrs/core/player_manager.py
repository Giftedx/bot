...
class PlayerManager:
            ...
            if player.state == PlayerState.MOVING:
                if self.movement.running:
                    player.run_energy = self.movement.update_energy(
                        player.run_energy,
                        player.skills[SkillType.AGILITY],
                        True
                    )
                else:
                    player.run_energy = self.movement.update_energy(
    ...
    async def create_player(self, username: str) -> Player:
        ...
        player = Player(
            id=player_id,
            username=username,
            position=Position(3222, 3218)  # Lumbridge spawn
        )
    ...
    async def load_player(self, player_id: int) -> Optional[Player]:
        ...
        bank = {
            item["item_id"]: item["quantity"]
            for item in await self.db.get_player_bank(player_id)
        }
        ...
        position = Position(
            ...
            location["y"],
            location["plane"]
        ) if location else Position(3222, 3218)
    ...
    async def _save_player(self, player: Player):
        ...
        for slot, item_id in vars(player.equipment).items():
            if item_id is not None:
                await self.db.update_player_equipment(
                    player.id,
                    slot,
                    item_id
                )
        ...
        # Update inventory
        await self.db.update_player_inventory(
            player.id,
            player.inventory
        )
        
        # Update bank
        await self.db.update_player_bank(
            player.id,
            player.bank
        )
    ...
    def get_player_by_username(self, username: str) -> Optional[Player]:
        ...
        """
        return next(
            (p for p in self.players.values() if p.username == username),
            None
        )
    ...
    def get_nearby_players(self,
                          position: Position,
                          max_distance: float) -> List[Player]:
    ...
    def can_access_content(self,
                          ...
                          members_only: bool = False) -> Tuple[bool, Optional[str]]:
