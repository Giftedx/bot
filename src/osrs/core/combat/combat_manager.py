...
class CombatManager:
    def __init__(self):
        ...
        self.weapon_styles = {
            ...
            'bow': [
                ...
                CombatStyle('Longrange', 'ranged', 'longrange', 'shared', {'ranged': 1, 'defence': 2}),
            ],
        }
    ...
    def calculate_hit(
        ...
        defence_roll = self.calculator.calculate_accuracy(
            defence_level,
            defender.equipment.defence_slash
        )
    ...
    async def process_combat_tick(self, state: CombatState) -> Optional[CombatEntity]:
        ...
        if state.attacker.is_player:
            state.attacker.special_attack_energy = min(
                100.0,
                state.attacker.special_attack_energy + 10
            )
        ...
        for hit in state.hits:
            if hit.get('is_special'):
                if hit['hit_lands']:
                    ...
                    log.append(
                        f"{hit['attacker']} misses {hit['defender']} with a special attack!"
                    )
                ...
                if hit['hit_lands']:
                    ...
                    log.append(
                        f"{hit['attacker']} misses {hit['defender']}!"
                    )
        return log 
