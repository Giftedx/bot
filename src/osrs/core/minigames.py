...
class MinigameManager:
    ...
    def __init__(self):
        ...
        self.fight_caves_waves = {
            ...
            # ... more waves ...
            63: [Enemy("TzTok-Jad", 702, 255, 97, "all", special_attacks=["ranged", "magic", "melee"])]
        }
        ...
        self.inferno_waves = {
            ...
            # ... more waves ...
            69: [Enemy("TzKal-Zuk", 1400, 600, 251, "all", special_attacks=["shield", "rangers", "mage"])]
        }
    ...
    async def process_minigame_tick(
        self,
        player_id: int,
    ...
    def _process_barbarian_assault(self, session: Dict, action: str) -> Dict:
        ...
        if action == "call":
            ...
            return {
                'message': f'Changed role to {new_role}!',
                'role': new_role
            }
    ...
    def _process_castle_wars(self, session: Dict, action: str) -> Dict:
        ...
        if action == "attack":
            ...
            if hit:
                ...
                return {
                    'message': f'Hit enemy for {damage} damage!',
                    'damage': damage
                }
            ...
            if success:
                return {
                    'message': f'Successfully used {item}!',
                    'item_used': item
                }
    ...
    async def _process_fight_caves(self, session: Dict, action: str) -> Dict:
        ...
        if action == "attack":
            ...
            return {
                'message': 'Restored prayer points',
    ...
    def _process_nightmare_zone(self, session: Dict, action: str) -> Dict:
        ...
        if action == "attack":
            ...
            return {
                'message': 'Drank absorption potion',
                'absorption': session['absorption']
            }
    ...
    def _process_tempoross(self, session: Dict, action: str) -> Dict:
        ...
        if action == "fish":
            ...
            if session['storm_intensity'] <= 0:
                ...
                self._end_session(session['player_id'])
                return {
                    'message': 'Tempoross has been defeated!',
                    'rewards': reward
                }
            ...
            if success:
                return {
                    'message': 'Successfully repaired the area!',
                    'energy': session['energy']
                }
            return {
                'message': 'Failed to repair the area!',
                'energy': session['energy']
            }
    ...
    def _process_volcanic_mine(self, session: Dict, action: str) -> Dict:
        ...
        if action == "prospect":
            ...
            if rock_type == 'unknown':
                ...
                return {
                    'message': f'Found {rock_type} deposits!',
                    'rock_type': rock_type
                }
            ...
            if vent_status == 'closed':
                ...
                return {
                    'message': 'Opened gas vent!',
                    'gas_levels': session['gas_levels']
                }
            else:
                session['vent_status'] = 'closed'
                return {
                    'message': 'Closed gas vent!',
                    'vent_status': 'closed'
                }
        ...
        if session['gas_levels'] >= 100:
            return {
                'message': 'Gas levels critical! Game Over.',
                'game_over': True
            }
            
        if session['stability'] <= 0:
            return {
                'message': 'Mine collapsed! Game Over.',
                'game_over': True
            }
    ...
    def _process_gauntlet(self, session: Dict, action: str) -> Dict:
        ...
        if time_remaining <= 0:
            return {
                'message': 'Time has run out! Game Over.',
                'game_over': True
            }
        ...
        if action == "gather":
            ...
            return {
                'message': f'Gathered {amount} {resource_type}!',
                'resources': resources
            }
    ...
    def _process_zalcano(self, session: Dict, action: str) -> Dict:
        ...
        if action == "mine":
            ...
            if success:
                ...
                session['ores'] += amount
                return {
                    'message': f'Mined {amount} tephra ores!',
                    'ores': session['ores']
                }
            ...
            if session['heat_level'] >= 100:
                ...
                return {
                    'message': 'Ores fully heated! Ready for smithing.',
                    'phase': 'smithing'
                }
            ...
            if session['boss_hp'] <= 0:
                ...
                self._end_session(session['player_id'])
                return {
                    'message': 'Zalcano has been defeated!',
                    'rewards': reward
                }
            ...
            if random.random() < 0.3:  # 30% chance for special attack
                ...
                if session['player_hp'] <= 0:
                    return {
                        'message': f'Killed by {attack_type}! Game Over.',
                        'game_over': True
                    }
    ...
    def _process_soul_wars(self, session: Dict, action: str) -> Dict:
        ...
        if action == "attack_players":
            ...
            if hit:
                damage = random.randint(0, 20)
                return {
                    'message': f'Hit enemy player for {damage} damage!',
                    'damage': damage
                }
            return {'message': 'Missed attack!'}
            ...
            if success:
                ...
                return {
                    'message': f'Captured the obelisk for {team} team!',
                    'obelisk': team
                }
            ...
            if avatar_hp[enemy_team] <= 0:
                ...
                self._end_session(session['player_id'])
                return {
                    'message': f'{team.title()} team wins!',
                    'rewards': reward
                }
                
            return {
                'message': f'Hit enemy avatar for {damage} damage!',
                'avatar_hp': avatar_hp
            }
            ...
            return {
                'message': f'Healed for {heal_amount} HP!',
                'hp': session['player_hp']
            }
    ...
    def _process_mahogany_homes(self, session: Dict, action: str) -> Dict:
        ...
        if action == "get_contract":
            ...
            session['contract'] = {
                ...
                'location': location,
                'repairs_needed': 3 if difficulty == 'novice' else 4 if difficulty == 'adept' else 5
            }
            ...
            return {
                'message': f'Added {amount} {material_type}!',
                'materials': materials
            }
    ...
    def _process_fishing_trawler(self, session: Dict, action: str) -> Dict:
        ...
        if time_remaining <= 0:
            ...
            self._end_session(session['player_id'])
            return {
                'message': f'Trip complete! Caught {fish_caught} fish!',
                'rewards': reward
            }
        ...
        if water_level >= 100:
            return {
                'message': 'The boat has sunk! Game Over.',
                'game_over': True
            }
            
        if net_status <= 0:
            return {
                'message': 'The net is completely destroyed! Game Over.',
                'game_over': True
            }
    ...
    def _process_trouble_brewing(self, session: Dict, action: str) -> Dict:
        ...
        if action == "get_water":
            ...
            return {
                'message': f'Collected {amount} buckets of water!',
                'resources': resources
            }
            ...
            return {
                'message': f'Collected {amount} sugar!',
                'resources': resources
            }
            ...
            return {
                'message': f'Collected {amount} hops!',
                'resources': resources
            }
        ...
        if random.random() < 0.1:  # 10% chance per tick
            ...
            if event == 'equipment_break':
                ...
                if resources['sugar'] > 0:
                    resources['sugar'] -= 1
                    return {
                        'message': 'A monkey stole some sugar!',
                        'resources': resources
                    }
            elif event == 'storm':
                if resources['water'] > 0:
                    ...
                    return {
                        'message': f'Storm spilled {lost} water!',
                        'resources': resources
                    }
    ...
    def _process_pyramid_plunder(self, session: Dict, action: str) -> Dict:
        ...
        if time_remaining <= 0:
            ...
            self._end_session(session['player_id'])
            return {
                'message': f'Time\'s up! Found {artifacts_found} artifacts!',
                'rewards': reward
            }
        ...
        if action == "search_urn":
            ...
            return {
                'message': f'Entered room {current_room + 1}!',
                'room': session['room']
            }
