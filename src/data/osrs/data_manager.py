...
class OSRSDataManager:
    ...
    def search_data(self, query: str, categories: List[str] = None) -> Dict[str, Any]:
        ...
        for category in search_categories:
            ...
            if isinstance(category_data, dict):
                ...
                matches = [item for item in category_data if query.lower() in str(item).lower()]
                if matches:
                    results[category] = matches
    ...
    def get_quest_info(self, quest_name: str) -> Optional[Dict[str, Any]]:
        ...
        return next(
            (q for q in quests.values() if q.get("name", "").lower() == quest_name.lower()), None
        )
    ...
    def get_skill_info(self, skill_name: str) -> Optional[Dict[str, Any]]:
        ...
        return next(
            (s for s in skills.values() if s.get("name", "").lower() == skill_name.lower()), None
        )
    ...
    def get_pet_info(self, pet_name: str) -> Optional[Dict[str, Any]]:
        ...
        return next(
            (p for p in pets.values() if p.get("name", "").lower() == pet_name.lower()), None
        )
    ...
    def get_diary_info(self, diary_name: str) -> Optional[Dict[str, Any]]:
        ...
        return next(
            (d for d in diaries.values() if d.get("name", "").lower() == diary_name.lower()), None
        )
    ...
    def get_music_info(self, track_name: str) -> Optional[Dict[str, Any]]:
        ...
        return next(
            (m for m in music.values() if m.get("name", "").lower() == track_name.lower()), None
        )

    def get_clan_info(self, clan_name: str) -> Optional[Dict[str, Any]]:
        """Get detailed information about a clan"""
        clans = self.data.get("clans", {})
        return next((c for c in clans if c.get("name", "").lower() == clan_name.lower()), None)
    ...
    def get_achievement(self, achievement_id: int) -> Optional[Dict[str, Any]]:
        ...
        return next((ach for ach in achievements if ach.get("id") == achievement_id), None)
