...
class DisplayFormatter:
    ...
    def format_stats(self, stats: Dict[str, int], include_xp: bool = True) -> Union[str, discord.Embed]:
    ...
    def format_combat(self, attacker_stats: Dict[str, int], defender_stats: Dict[str, int],
                     hit_chance: float, max_hit: int) -> Union[str, discord.Embed]:
    ...
    def format_drop_table(self, drops: List[Dict[str, any]]) -> Union[str, discord.Embed]:
        ...
        if self.config.mode == DisplayMode.TEXT:
            lines = []
            for drop in drops:
                chance = f"1/{int(1/drop['chance'])}" if drop['chance'] < 1 else f"{drop['chance']*100}%"
    ...
    def format_location(self, area_data: Dict[str, any]) -> Union[str, discord.Embed]:
        ...
        if self.config.mode == DisplayMode.TEXT:
            ...
            if area_data.get('requirements'):
                reqs = "\n".join(f"{req}: {value}" for req, value in area_data['requirements'].items())
    ...
    def format_skill_progress(self, skill: str, level: int, xp: int, 
                            next_level_xp: int) -> Union[str, discord.Embed]:
...
class DisplaySystem:
    def __init__(self, 
        ...
        self.viewport = ViewPort(
            width=31,  # Odd numbers work better for centering
            height=15,
            center_x=0,
            center_y=0
        )
        ...
        for y in range(min_y, max_y + 1):
            ...
            for x in range(min_x, max_x + 1):
                if x == player_x and y == player_y:
                    ...
                    tile = self.movement.current_area.tiles[y][x] if 0 <= y < len(self.movement.current_area.tiles) and 0 <= x < len(self.movement.current_area.tiles[0]) else None
    ...
    async def render_graphical_mode(self) -> Dict[str, any]:
        ...
        for y in range(min_y, max_y + 1):
            for x in range(min_x, max_x + 1):
                if 0 <= y < len(self.movement.current_area.tiles) and 0 <= x < len(self.movement.current_area.tiles[0]):
