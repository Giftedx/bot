...
class EnhancedDataManager:
    ...
    async def fetch_wiki_data(self, query: str, props: List[str] = None) -> Dict[str, Any]:
        ...
        params = {"action": "query", "format": "json", "titles": query, "prop": "|".join(props)}
    ...
    async def fetch_current_prices(self, item_ids: List[str]) -> Dict[str, int]:
        ...
        try:
            async with self.session.get(
                f"{OSRSEndpoints.PRICES_API}/latest", params=params
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def fetch_hiscores(self, username: str) -> Optional[Dict[str, int]]:
        ...
        try:
            async with self.session.get(
                f"{OSRSEndpoints.HISCORES_API}?player={username}"
            ) as response:
                if response.status == 200:
    ...
    async def calculate_pet_chance(
        self, pet_name: str, player_stats: Dict[str, int], modifiers: Dict[str, float] = None
    ) -> Dict[str, float]:
    ...
    def _calculate_combat_level(self, stats: Dict[str, int]) -> int:
        ...
        range_magic = 0.325 * (
            math.floor(3 * stats.get("ranged", 1) / 2) + math.floor(3 * stats.get("magic", 1) / 2)
        )
            ...
            if pet.requirements:
                for skill, level in pet.requirements.items():
                    stats["skill_requirements"][skill.value] = max(
                        stats["skill_requirements"][skill.value], level
                    )
    ...
    async def _process_boss_data(self, data: Dict[str, Any]) -> OSRSBoss:
        ...
        try:
            return OSRSBoss(
                ...
                recommended_stats=await self._process_combat_stats(
                    data.get("recommended_stats", {})
                ),
            )
        except Exception as e:
