...
class AchievementManager:
    ...
    async def _check_training_achievements(self, event: GameEvent) -> None:
        ...
        if pet_type == "osrs":
            ...
            if "combat_level" in event.data:
                user_data["osrs_combat_level"] = max(user_data.get("osrs_combat_level", 0), 
                                                   event.data["combat_level"])
    ...
    async def _check_achievements(self, user_id: str, user_data: Dict[str, Any], 
                                achievement_type: Optional[AchievementType] = None) -> None:
        """Check if any achievements have been completed"""
        achievements = (achievement_config.get_achievements_by_type(achievement_type) 
                      if achievement_type else achievement_config.achievements.values())
    ...
    async def _grant_achievement(self, user_id: str, achievement: Achievement) -> None:
        ...
        await self.db_service.add_achievement(user_id, {
            "id": achievement.id,
    ...
    async def get_achievement_progress(self, user_id: str) -> Dict[str, Any]:
        ...
        for ach in achievement_config.achievements.values():
            if ach.id in completed_ids:
                ...
                met_reqs = sum(1 for req, value in ach.requirements.items()
